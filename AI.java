...
public class AI implements DecisionMaker {
  ...
  private String use_arg() {
    String thing = NPC.randomArtifact(character.current.shuffleArtifacts());
    if (thing == null)
      thing = "none";
    return thing;
  }
  ...
  private Move execCommand() {
    ...
    if ("GO".equalsIgnoreCase(str)) {
      ...
      if (arg == null)
        return null;
      // System.out.println(character.name()+" getting "+arg+" in
      ...
      if (arg == null)
        return null;
      // System.out.println(character.name()+" using "+arg+" in
      // "+character.current.name());
      return new Use(character, arg);
    }

    else if ("DROP".equalsIgnoreCase(str)) {
      if (character.name().equals("A Cheerful Leprechaun"))
        return null;
      arg = use_arg();
      if (arg == null)
        return null;
      // System.out.println(character.name()+" Dropping "+arg+" in
      // "+character.getCurrentPlace().name());
